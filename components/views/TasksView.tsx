      ...
      <SectionTitle
        ...
        right={
          <div className="flex items-center gap-2">
            {permissions.canCreateEvents && <Button title="New Task" onClick={onNewEvent}><Plus className="w-4 h-4 mr-2" />New Task</Button>}
            {permissions.canManageTemplates && <Button title="Templates & Categories" variant="ghost" onClick={onOpenTemplates}>Templates & Categories</Button>}
          </div>
        }
      >
              ...
              {events.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()).map((e) => {
                ...
                return (
                  <tr key={e.id} className="border-b border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50/40 dark:hover:bg-zinc-800/40 transition-colors">
                    ...
                    <td className="px-4 py-3 text-zinc-600 dark:text-zinc-300">{fmtDate(e.date)}</td>
                    ...
                    <td className="px-4 py-3">
                      <Badge colorClass={getStatusBadgeColor(e.status)}>{e.status}</Badge>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        {e.status !== 'Completed' && (
                          <Button title="Mark as completed" size="sm" variant="outline" onClick={() => onUpdateEvent(e.id, { status: 'Completed' })}>
                            <CheckCircle2 className="w-4 h-4" />
                          </Button>
                        )}
                        {permissions.canEditEvents && (
                            <Button title="Edit" size="sm" variant="ghost" onClick={() => onEditEvent(e)}><Edit className="w-4 h-4"/></Button>
                        )}
                        {permissions.canDeleteEvents && (
                            <Button title="Delete" size="sm" variant="ghost" className="text-zinc-500 hover:bg-rose-100 hover:text-rose-600 dark:text-zinc-400 dark:hover:bg-rose-900/40 dark:hover:text-rose-300" onClick={() => onRemoveEvent(e.id)}><Trash2 className="w-4 h-4" /></Button>
                        )}
                      </div>
                    </td>
                  </tr>
